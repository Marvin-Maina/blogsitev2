...
export default function WriterDashboard() {
  ...
  const handleEdit = (id) => {
    const storedArticles = JSON.parse(localStorage.getItem("allArticles")) || [];
    const articleToEdit = storedArticles.find((article) => article.id === id);

    if (articleToEdit) {
      localStorage.setItem("editingArticle", JSON.stringify(articleToEdit));
      navigate("/ContentEditor");
    }
  };
  ...
  const handleDelete = (id) => {
    ...
    fetchWriterArticles(); // Refresh dashboard
  };
          ...
          articles.map((article) => (
            ...
            >
              <div className="flex flex-col w-3/4">
                ...
                >
                  {getTextOnly(article.content)?.substring(0, 150)}...
                </p>
