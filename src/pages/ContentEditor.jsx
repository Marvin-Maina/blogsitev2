...
export default function ContentEditor() {
  ...
  const saveArticle = (articleStatus) => {
    ...
    const articleData = {
      ...
      image: extractFirstImage(contentInput),
    ...
    <div className={`p-6 min-h-screen font-sans transition-colors duration-300 ${theme.bg} ${theme.text}`}>
      <div className="flex flex-col md:flex-row h-screen p-6 ml-20">
        ...
        <div className="w-full md:w-1/2 p-4">
          ...
          <div
            className={`p-4 rounded-lg overflow-auto h-96 ${theme.preview} ${theme.border} border prose prose-invert dark:prose-invert`}
      ...
      {/* Media Library Modal */}
      {showMediaLibrary && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-60 flex justify-center items-center transition-opacity duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {mediaFiles.map((file, index) => (
                ...
                >
                  ...
                  />
                  <p className="mt-2 text-sm text-center text-gray-700 dark:text-gray-300">{file.name}</p>
                </div>
              ))}
            </div>
            <button
