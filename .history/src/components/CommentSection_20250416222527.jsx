...
const CommentSection = ({ articleId }) => {
  ...
  const [comments, setComments] = useState([]); // Start with an empty array as default
  const [text, setText] = useState('');
  const currentUser = JSON.parse(localStorage.getItem('currentUser'));

  useEffect(() => {
    const storedComments = JSON.parse(localStorage.getItem('comments')) || {}; // Get comments from localStorage
    const articleComments = storedComments[articleId] || []; // Ensure it's an array
    if (Array.isArray(articleComments)) {
      setComments(articleComments); // Set the comments if it's an array
    } else {
      setComments([]); // If it's not an array, default to an empty array
    }
  }, [articleId]);
