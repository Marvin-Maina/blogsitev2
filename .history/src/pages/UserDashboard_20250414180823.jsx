        ...
        <ul className="space-y-3">
          {recentActivity.length === 0 ? (
            ...
            recentActivity.map((activity) => (
              <li
                key={activity.title}
                className={`p-4 rounded-lg border ${isDarkMode ? 'bg-gray-800 border-gray-700 text-gray-200' : 'bg-gray-100 border-gray-300 text-gray-900'}`}
              >
        ...
        {userArticles.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {userArticles.map((article) => (
              <li
                key={article.title}
                className={`p-5 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-[1.02] border ${isDarkMode ? 'bg-gray-800 bg-opacity-90 border-purple-800' : 'bg-white bg-opacity-90 border-purple-300'}`}
              >
                <img
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    className={`w-full p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 ${isDarkMode ? 'bg-gray-800 text-white placeholder-gray-400' : 'bg-white text-gray-900 placeholder-gray-500'}`}
                    rows="3"
