...
export default function ProfilePage() {
  ...
  const [profile, setProfile] = useState({
    name: user?.name || "",
    email: user?.email || "",
    avatar: user?.avatar || "", // Default avatar logic
  });
  const [isEditing, setIsEditing] = useState(false);

  const { isDarkMode } = useContext(ThemeContext); // Get dark mode state

  // Sync the profile with the user context when it changes
  useEffect(() => {
    setProfile({
      name: user?.name || "",
      email: user?.email || "",
  ...
  }, [user]); // Only update when the user context changes
  ...
  const handleProfileUpdate = () => {
    // Ensure there's actually a change in profile before saving
    if (
      ...
      <div className={`${isDarkMode ? "bg-gray-800" : "bg-white"} p-6 rounded-lg shadow-md mb-10`}>
        <div className="flex items-center gap-4">
          ...
          <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-300">
            <img
              src={profile.avatar || "/default-avatar.png"} // Use default avatar if not set
              alt="Profile"
