...
export default function ContentEditor() {
  ...
  const [selectedImageURL, setSelectedImageURL] = useState(null); // To store the selected image URL
  ...
  const saveArticle = (articleStatus) => {
    ...
    const articleData = {
      ...
      image: selectedImageURL || null, // Store selected image URL
    };
    ...
    setContentInput("");
    setStatus("draft");
    setSelectedImageURL(null); // Reset image after saving
  };
    ...
    <div className={`p-6 min-h-screen font-sans transition-colors duration-300 ${theme.bg} ${theme.text}`}>
      <div className="flex flex-col md:flex-row h-screen p-6 ml-20">
        ...
        <div className={`w-full md:w-1/2 p-4 ${theme.border} border`}>
          ...
          <div className="mt-4">
            <button
              onClick={() => {
                ...
                // Set selectedImageURL after image is chosen
              }}
          ...
          {selectedImageURL && (
            <div className="mt-4">
              <img src={selectedImageURL} alt="Selected" className="w-full h-40 object-cover rounded-lg" />
            </div>
          )}
