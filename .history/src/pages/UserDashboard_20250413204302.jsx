...
export default function UserDashboard() {
  ...
  // Fetch articles from shared `allArticles` localStorage key
  useEffect(() => {
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {newsArticles.map((article) => (
              ...
              >
                ...
                {/* Rating */}
                <div className="mt-4">
                  <h4 className="font-semibold text-sm">Rate this article:</h4>
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        key={star}
                        className={`text-${star <= (ratings[article.title] || 0) ? 'yellow' : 'gray'}-400`}
                        onClick={() => handleRatingChange(article.title, star)}
                      >
                        ⭐
                      </button>
                    ))}
