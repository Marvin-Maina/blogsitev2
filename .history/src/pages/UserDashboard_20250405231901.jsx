  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        if (data.status === 'ok') {
          setNewsArticles(data.articles.slice);
        } else {
      ...
      {loading ? (
        ...
        <ul className="mt-4 space-y-3">
          {newsArticles.map((article, index) => (
            <li key={index} className="bg-gray-800 p-4 rounded-lg transition-all duration-300 ease-in-out transform hover:scale-105 hover:bg-gray-700">
              {/* Image */}
              {article.urlToImage && (
                <img
