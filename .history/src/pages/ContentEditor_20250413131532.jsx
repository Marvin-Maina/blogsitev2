...
import { Bold, Italic, Heading, Code, List, Link, Save, Image, CheckCircle } from "lucide-react";
...
export default function ContentEditor() {
  ...
  const handleSave = (forcedStatus = status) => {
    setSaving(true);
    const articleData = {
      title,
    ...
    const action = forcedStatus === "published" ? "Published" : "Saved";
    setToastMessage(`Article ${action} Successfully!`);
    setTimeout(() => setToastMessage(""), 3000); // Clear toast after 3s
  };
      ...
      <div className={`flex flex-col md:flex-row h-screen ${theme.bg} ${theme.text} p-6 ml-20`}>
        ...
        <div className={`w-full md:w-1/2 p-4 ${theme.border} border`}>
          ...
          <button
            ...
            className="mt-2 px-4 py-2 bg-green-600 text-white rounded-lg flex items-center gap-2 hover:bg-green-500 transition"
            disabled={saving}
          >
          ...
          {toastMessage && (
            <div className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg flex items-center gap-2 shadow-md transition-all duration-300 animate-fade-in">
              <CheckCircle size={20} />
