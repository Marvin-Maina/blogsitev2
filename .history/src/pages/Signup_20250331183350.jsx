...
export default function Signup() {
  ...
  async function handleSignup() {
    ...
    // Wait for Supabase session creation
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Fetch user from the session instead of getUser()
    const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
    const user = sessionData?.session?.user;

    if (!user) {
      console.error("Failed to fetch user after signup:", sessionError);
      alert("Signup successful, but user session couldn't be verified.");
      setLoading(false);
      return;
    }
