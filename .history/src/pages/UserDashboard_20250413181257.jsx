...
export default function UserDashboard() {
  ...
  const [newsArticles, setNewsArticles] = useState([]);  // Articles from News API
  const [userArticles, setUserArticles] = useState([]);  // Articles from local writers
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

 // Line 1 - 12 stays the same...

// ✅ Inside useEffect for fetching user articles
useEffect(() => {
  const fetchUserArticles = () => {
    const savedArticles = JSON.parse(localStorage.getItem('allArticles')) || [];

    // Filter articles: must be published & not by the logged-in user
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        if (data.status === 'ok') {
          setNewsArticles(data.articles.slice(0, 6));  // Get top 6 articles from News API
        } else {
  ...
  // Fetch articles written by local writers (other than the logged-in user)
  useEffect(() => {
    const fetchUserArticles = () => {
      const savedArticles = JSON.parse(localStorage.getItem('articles')) || [];

      // Filter articles written by "writers" and exclude articles from the current user
      const localWriterArticles = savedArticles.filter(
        (article) => article.status === 'published' && article.author !== user.name
      );
      setUserArticles(localWriterArticles);  // Set articles written by local writers (other than the logged-in user)
    };
    fetchUserArticles();
  }, [user]);  // Depend on user to re-fetch when user changes (if needed)
